...
exports.getOnePost = (req, res, next) => {
  let getOneQuery = `SELECT users.prenom, users.imageurl, posts.title, posts.image_url, posts.numero, posts.link, posts.description, TIMEDIFF(NOW(),Posts.date) as date FROM posts INNER JOIN users ON posts.user_id = users.id WHERE posts.numero =${req.params.id}`;
  database.query(getOneQuery, function (err, result) {
    if (err) throw err;
      ...
      if(result.length > 0) {
        ...
        let getLikesQuery = `SELECT users.prenom as usersLike FROM users INNER JOIN likes ON likes.user_id = users.id WHERE likes.post_id =${req.params.id}`;
        database.query(getLikesQuery, function (err, result) {
          if (err) throw err;
            ...
            let getDislikesQuery = `SELECT users.prenom as usersDislike FROM users INNER JOIN dislikes ON dislikes.user_id = users.id WHERE dislikes.post_id =${req.params.id}`;
...
exports.getPostComments = (req, res, next) => {
  let getCommentQuery = `SELECT users.prenom, users.imageurl, comments.numero, comments.comment FROM comments INNER JOIN users ON comments.user_id = users.id WHERE comments.post_id =${req.params.id} ORDER BY comments.date DESC`;
...
exports.modifyPost = (req, res, next) => {
  ...
  if (req.file) {
    const postObject = JSON.parse(req.body.post);
    const infoMediaUrlQuery = "SELECT image_url FROM posts where numero = "+ req.params.id;
    ...
    database.query(infoMediaUrlQuery, function (err, result) {
      if (err) {res.status(400).json({ error : err.code });}
        ...
        const putQuery = `UPDATE Posts SET title = "${postObject.title}", description = "${postObject.description}", image_url = "${mediaUrl}", link = "${postObject.url}" where numero = ${req.params.id}`;
  ...
  // Cas sans média dans la requête PUT
    ...
    const putQuery = `UPDATE Posts SET title = "${req.body.title}", description = "${req.body.description}", image_url = "${req.body.mediaUrl}" , link = "${req.body.url}" where numero = ${req.params.id}`;
